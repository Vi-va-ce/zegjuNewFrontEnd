...
function ChoicesTwo({ data, currentIndex, onLocalStorageChange }) {
  ...
  useEffect(() => {
    ...
    if (
      data &&
      data.sections?.[1]?.section_id &&
      data.sections?.[1]?.question_group?.[0]?.question_types?.[0]?.questions?.[currentIndex]?.question_text
    ) {
      const sectionId = data.sections[1].section_id;
      const question =
        data.sections[1].question_group[0].question_types[0].questions[currentIndex];
  ...
  const handleClick = (choiceId) => {
    if (selected === choiceId) {
      ...
      const sectionId = data.sections[1].section_id;
      const questionId = data.sections[1].question_group[0].question_types[0].questions[currentIndex].question_id;
      const testId = data.test_id;
