...
function TestModes2({ subject, onPracticeData }) {
  const navigate = useNavigate();

  useEffect(() => {
    if (subject) {
      axios.post('https://zegeju-1453f.uc.r.appspot.com/api/v11/student/GetQuestionUatFree', {
        subjectId: subject,
      })
        .then(response => {
  ...
  const handlePracticeClick = () => {
    navigate('/UAT/TestModes2/practice2');
  };
const handleExamClick= () => {
  navigate('/UAT/TestMode2/Exam2');
};

console.log()
  return (
    <div className='bg-[#ecf7f8] h-screen'>
      <button className='pl-[12px] pt-[12px] transition-transform duration-300 hover:scale-110' onClick={() => navigate('/UAT2')}>
        <img src={backb} />
      </button>
