...
function TestMode({ subject, onPracticeData }) {
  const navigate = useNavigate();

  useEffect(() => {
    if (subject) {
      axios.post('v11/student/GetQuestionGat', {
        subjectId: subject,
      })
        .then(response => {
  ...
  const handlePracticeClick = () => {
    navigate('/GAT/TestMode/practice');
  };
const handleExamClick= () => {
  navigate('/GAT/TestMode/Exam');
};
  return (
    <div className='bg-[#ecf7f8] h-screen'>
      <button className='pl-[12px] pt-[12px] transition-transform duration-300 hover:scale-110' onClick={() => navigate('/GAT')}>
        <img src={backb} />
      </button>
