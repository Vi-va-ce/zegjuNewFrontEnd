...
function PracticePro({ subjectName, message }) {
  ...
  const fetchData = async () => {
    try {
      let response;
      if (message === "full") {
        response = await axios.post('v10/student/GetQuestionProPaid', {
          subjectId: subjectName,
        });
      } else {
        response = await axios.post('v10/student/GetQuestionFree', {
          subjectId: subjectName,
        });
      }
